...
def _preprocess_data(args):
    ...
    for full_wav in full_wavs:
        ...
        for i in range(len(mfcc_feats)):
                ...
                region.append(dir_list[-3])
                speaker.append(dir_list[-2])
                sentence.append(dir_list[-1].split("_")[0])
        ...
        mfcc_region.extend(region)
        mfcc_speaker.extend(speaker)
        mfcc_sentence.extend(sentence)
    ...
    timit_df["region"] = mfcc_region
    timit_df["speaker"] = mfcc_speaker
    timit_df["sentence"] = mfcc_sentence
    if (n_delta == 0):
